...
class TestAddRace(unittest.TestCase):
    ...
    def test_add_race_form_too_many_races(self, username):
        with self.client as client:
            username.is_authenticated = True
            client.post('/rounds/1/races/add',
                        data=dict(race_status="TEST STATUS", race_date="10/25/2018 12:00 AM"))

            with client.session_transaction() as session:
                flash_message = session['_flashes'][0][1]
                self.assertEqual(
                    flash_message, "Can't add race to round with 5 or more races.")
    ...
    def test_add_race_form_too_early(self, username):
        with self.client as client:
            username.is_authenticated = True
            client.post('/rounds/1/races/add',
                        data=dict(race_status="TEST STATUS", race_date="10/25/2016 12:00 AM"))

            with client.session_transaction() as session:
                flash_message = session['_flashes'][0][1]
                self.assertEqual(
                    flash_message, "Can't add race that doesn't take place within round dates.")
    ...
    def test_add_race_form_too_late(self, username):
        with self.client as client:
            username.is_authenticated = True
            client.post('/rounds/1/races/add',
                        data=dict(race_status="TEST STATUS", race_date="10/25/2020 12:00 AM"))
    ...
    def test_add_race_same_time(self, username):
        with self.client as client:
            username.is_authenticated = True
            client.post('/rounds/1/races/add',
                        data=dict(race_status="TEST STATUS", race_date="05/05/2018 12:00 AM"))

            with client.session_transaction() as session:
                flash_message = session['_flashes'][0][1]
                self.assertEqual(
                    flash_message, "Two races cannot start at the same time.")
